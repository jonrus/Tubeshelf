...
- Subscribe by `@handle` or channel URL directly, resolved to a channel ID by scraping the channel page's canonical/meta tags server-side (rather than requiring the user to find the ID themselves as in MVP) (refined in docs/specs/016-handle-url-subscribe.md)
...
- `PUID`/`PGID` env var support (linuxserver.io-style) so the container can run as an arbitrary host UID/GID and avoid bind-mount permission friction on the `/data` volume. MVP deployment docs instead just document manually `chown`-ing the data directory to match the container's user before first run (refined in docs/specs/017-puid-pgid-support.md)
- Graceful shutdown (SIGTERM handling) for the HTTP server and the background RSS-fetch scheduler, so a `docker compose down`/restart can't kill an in-flight fetch mid-write. Deferred since SQLite's WAL mode makes this low-risk as-is (durable on commit) (refined in docs/specs/023-graceful-shutdown.md)
- A broader structured-logging pass across the app (log levels, what's signal vs. noise for ingestion/ignore-rule processing, etc.), beyond the narrowly-scoped startup-migration-failure message the deployment spec adds (refined in docs/specs/028-structured-logging.md — a shared logger module, `LOG_LEVEL`/`LOG_FORMAT` config, and a `TZ` env var for both log timestamps and UI date display; deliberately scoped to cleaning up existing call sites only, not adding net-new coverage like ignore-rule-processing logging, which stays deferred)
- Re-evaluate turning `fallow`'s `typeAware` analysis back on (`typeAware.enabled: true` in `.fallowrc.json`, currently left off — refined in docs/specs/026-fallow-adoption.md). Deferred because enabling it during spec026 didn't measurably change this repo's dead-code findings (checked directly: identical `unused-exports`/`unused-types` counts either way at 64 files) while costing an ongoing devcontainer exception (its sidecar needs real Node.js, which the `oven/bun:1` devcontainer base image lacks, and hangs forever without it). Worth revisiting if the codebase grows enough that type-aware reachability starts actually diverging from the non-type-aware pass — and if so, pair it with adding Node.js to the devcontainer image (e.g. the `ghcr.io/devcontainers/features/node:1` feature, following the same pattern already used for `git`/`github-cli`) so the exception can be closed rather than reopened
...
- **CI/CD Pipeline:** TBD - probably whatever I can get away with for free on GitHub (refined in docs/specs/015-github-buildout.md; static analysis/dead-code checks added in docs/specs/026-fallow-adoption.md)
