...
5. Users can unsubscribe from a channel. Doing so deletes that channel's Unwatched/Watching/**Ignored** videos, but videos already marked Watched are kept as history.
...
- **Channel:** A subscribed YouTube channel; belongs to exactly one Category (defaults to "Uncategorized" if none chosen at subscribe time). Unsubscribing deletes the channel's Unwatched/Watching/Ignored videos but keeps Watched ones as history. Flagged `possible_missed_videos: bool` (manually dismissed) per the RSS gap-detection check in *Ingestion Notes*.
...
- **Relationships:**
  - Category 1-to-many Channel (one category has many channels; each channel has exactly one category)
  - Channel 1-to-many Video (one channel has many videos)
  - User 1-to-many Channel (modeled now for future multi-user support; single implicit user for MVP)
